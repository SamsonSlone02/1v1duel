...
bool PhysWorld::addObject(Object * in_object){
    ...
    for(int i = 0; i < arrSize;i++)
    {
        if(objectArr[i] == NULL)
        {
            ...
            std::cout << "Adding" <<  in_object << std::endl;
            std::cout << "into slot " << i << std::endl;
            return true;
...
bool PhysWorld::remObject(Object * in_object) {
    ...
    for(int i = 0; i < arrSize;i++)
    {
        ...
        {
            std::cout << "Removing " << in_object << std::endl;
            std::cout << "from slot " << i << std::endl;
            //delete objectArr[i];
            objectArr[i] = NULL;
            return true;
        }
...
void Boomerang::fireWeapon()
{
    ...
    if (ts > 1.5) {
        ...
        if (nbullets < MAX_BULLETS) {
            ...
            b = &barr[nbullets];
            std::cout << member->addObject(b) << std::endl;
            //shoot a bullet...
            //cout << &b << endl;		
            timeCopy(&b->time, &bt);
            //b->bulletParent = this;
...
void Boomerang::physics()
{
    ...
    while (i < nbullets) {
        ...
        if (ts > ttl) {
            ...
            b->xBounce = 1;
            b->yBounce = 1;
            std::cout << member->remObject(b) << std::endl;
